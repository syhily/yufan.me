...
todos:
  ...
  - id: rename-page-to-metric
    content: 现有 page 计数器表的业务命名全量改为 metric（schema.ts 物理名仍为 page；导出 pageTable 但业务侧 import as metricTable）。重命名 src/server/db/query/page.ts→metric.ts、所有 pageKey→metricKey、pageMetricsByKeys→metricsByKeys、upsertPage→upsertMetric 等；server/shared/UI/routes/tests 全量同步。
    status: pending
  - id: shared-types
