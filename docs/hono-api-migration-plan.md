...
| PR | 内容 | 风险 |
|---|---|---|
| A1 | spike：安装 Hono + ts-rest 依赖；空 `server/http/` 骨架；建 `server.ts`（`react-router-hono-server` 入口）；调整 `vite.config.ts`；**所有请求仍交给 RR**，仅验证外壳层不破坏现有功能 | 中（dev server 整合可能踩坑） |
...
2. `src/server.ts` 最小实现（`react-router-hono-server` 包 RR，无任何 API）
...
8. **决策文档**（保存为 `docs/hono-api-migration-plan.md` 的更新注释）：
   - HMR 体感（迁移前 vs 后秒级对比）
   - 构建产物大小变化
   - 启动时间变化
   - 踩到的坑与对策
...
- **Phase A（基建）**：`react-router-hono-server` 已落地，`server/http/` 骨架完整，
  middleware（session / install-gate / wp-decoy / visitor-cookie / request-id）全部迁到 Hono。
- **Phase B（API 端点迁移）**：所有域的契约 + controller 已实现；客户端调用点全部迁移到
  ts-rest client (`api` + `unwrap` + `useApiQuery` / `useApiMutation`)。
- **Phase C（资源路由）**：`feedRouter`、`imagesRouter`、`sitemapRouter` 已上线，URL 字节级保留。
- **Phase D（清理）**：
  - 旧文件已删除：`shared/api-actions.ts`、`api-envelope.ts`、`api-types.ts`、
    `client/api/fetcher.ts`、`client/api/submit.ts`、
    `client/api/use-admin-mutation.ts`、`client/api/music-loader.ts`、
    `client/api/render-math-fetch.ts`、`client/api/render-mermaid-fetch.ts`、
    `server/middleware/*`。
  - `routes.ts` 已简化到 76 行（< 80）。
  - `AGENTS.md` 已按附录 B 更新（API Layer、Permission Matrix、防御性约束、Session 描述）。
  - 新增契约测试 `tests/contract.apiContract.test.ts` + 类型测试 `tests/type.contract.test-d.ts`。
  - Legacy `/api/actions/*` 路径在 `server.ts` 中返回 301（GET）或 410（mutation）。

### 已知技术债务

1. **Controller 类型安全**：当前所有 controller 函数签名使用 `args: any, ctx: any`，
   `server/http/app.ts` 中挂载时需要 `as any`。要完全修复，需要给每个 controller
   显式标注 `ContractImpl<typeof ...Contract>` 并使用契约推导的参数解构。
2. **契约 schema 精度**：大量 admin 契约的 `body` / `query` 仍使用 `z.any()` 占位符，
   未切换到精确的 Zod schema。这不会导致运行时错误（adapter 的 `validate` 会透传），
   但失去了契约的输入校验价值。
3. **历史注释已清理**：代码库中所有引用 `useApiFetcher`、`API_ACTIONS`、
   `defineApiAction`、`api-types`、`api-envelope` 的注释均已更新或删除；
   `src/routes/api/` 空目录已移除。
