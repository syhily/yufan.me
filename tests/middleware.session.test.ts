...
// instance. The middleware reaches the real resolver through
// `@/server/auth/primitives`, so we stub the underlying primitives
// (Redis + storage); the resolver still runs once per request, which is
// what we ultimately want to assert.
...
const { requestContext, sessionContext, getRouteRequestContext } = await import('@/server/session')
