...
// `@/server/db/query/setting` here, and Vitest does not re-mock modules
// imported from a setup file (they're already cached by the time a
// test file's `vi.mock(...)` is hoisted) — so reaching for the slot
// from `@/shared/blog-config` keeps the DB query module unloaded until
...
import { BLOG_SETTINGS_SNAPSHOT_SLOT } from '@/shared/blog-config'
