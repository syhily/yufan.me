...
//
// **Key shape** `cms-post-draft:<postId>:<clientRevisionToken>`
//
...
//     localStorage works.
//   - `BroadcastChannel('cms-post-draft')` — modern, post-message
...
export interface UsePostLocalDraftResult {
  ...
  /** Drop the LS slot for the current `(postId, revToken)` pair. */
  clearDraft: () => void
}
...
export function usePostLocalDraft({
  ...
  // `(postId, revToken)` pair so a save (which writes a fresh slot)
  ...
  //   - A shared `BroadcastChannel` named `cms-post-draft` so a
