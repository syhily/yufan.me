import type { AdminListFilters, PendingCommentRow } from '@/db/query/comment.server'
...
} from '@/db/query/comment.server'
import { findPageByKey, upsertPage } from '@/db/query/page.server'
import { insertCommentUser, updateLastLogin } from '@/db/query/user.server'
import { DomainError } from '@/schemas/errors'
import { isAdmin, userSession, type BlogSession } from '@/services/auth/session.server'
import { sendApprovedComment, sendNewComment, sendNewReply } from '@/services/email/sender.server'
...
export async function loadComments(
  session: BlogSession | null | undefined,
  key: string,
  title: string | null,
  offset: number,
  ...
  const [, pendingArray] = await Promise.all([
    ...
    Promise.resolve(session ? (isAdmin(session) ? [false, true] : [false]) : [false]) as Promise<boolean[]>,
  ])
...
export async function increaseViews(key: string, title: string | null) {
  ...
  if (import.meta.env.PROD) {
    ...
    const { bumpPageView } = await import('@/services/metrics/batcher.server')
    bumpPageView(key)
  }
}
