import { Body, Container, Html, Img, Link, Section, Text } from '@react-email/components'
      ...
      <Body style={body}>
        <Container style={container}>
          ...
          <Section style={{ textAlign: 'center', padding: 20 }}>
            <Link href={config.website} target="_blank" style={ctaButton}>
              访问博客
            </Link>
          </Section>
          <Section style={{ textAlign: 'center', padding: 20 }}>
            <Text style={footer}>
              本邮件为系统自动发出，无法回复。
              <br />© {year}{' '}
              <Link href={config.website} style={footerLink} target="_blank">
                {config.title}
              </Link>
            </Text>
          </Section>
