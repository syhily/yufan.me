...
/**
 * Return the structured TOC entries this body would render. `slug` is
 * generated by `github-slugger` so collisions across multiple `## Foo`
 * headings disambiguate as `foo-1`, `foo-2`, … — same algorithm
 * `rehype-slug` uses on the historical MDX path, so heading anchors
 * stay stable when a page migrates from MDX into the new editor.
 */
export function collectHeadings(body: PortableTextBody): PortableTextHeading[] {
