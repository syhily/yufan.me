...
// 渲染分页（使用与 Pagination.astro 相同的样式）
function renderPagination(): void {
  ...
  // 转换为 1-based 页码（Pagination 组件使用 1-based）
  const currentPageNum = currentPage + 1

  let paginationHtml = '<nav class="navigation pagination" aria-label="评论">'
  paginationHtml += '<h2 class="screen-reader-text">评论导航</h2>'
  paginationHtml += '<div class="nav-links">'

  // 根据总页数选择渲染方式（参考 Pagination.astro 的逻辑）
  if (totalPages <= 6) {
    // 小分页：显示所有页码
  ...
  // 绑定分页事件
  paginationContainer.querySelectorAll('.page-numbers[data-page]').forEach((link) => {
    link.addEventListener('click', (e) => {
      e.preventDefault()
      const page = Number.parseInt((link as HTMLElement).dataset.page || '0')
