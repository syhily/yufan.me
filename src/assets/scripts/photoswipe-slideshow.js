...
class PhotoSwipeSlideshow {
  ...
  init(pswp) {
    ...
    pswp.on('uiRegister', () => {
      ...
      pswp.events.add(document, 'keydown', (e) => {
        switch (e.code) {
          ...
          case 'NumpadAdd':
          case 'Equal':
            this.changeSlideshowLength(1000, pswp)
            e.preventDefault()
            break
    ...
    this.lightbox.on('change', () => {
      if (this.slideshowIsRunning && this.options.restartOnSlideChange) {
        this.goToNextSlideAfterTimeout(pswp)
      }
    })
  ...
  changeSlideshowLength(delta, pswp) {
  ...
  goToNextSlideAfterTimeout(pswp) {
    // Reset the progress bar and timer.
    this.resetSlideshow()

    if (this.slideContentHasLoaded(pswp)) {
      // Get timeout length, accounting for various media types.
      const currentSlideTimeout = this.getSlideTimeout(pswp)
