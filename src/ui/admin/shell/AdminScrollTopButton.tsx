...
export interface AdminScrollTopButtonProps {
  ...
  /**
   * When `true`, the FAB rides higher than its default bottom-right
   * slot to clear another bottom-right FAB (today: the editor's
   * `FloatingPublishButton`). `AdminShell` ORs this from
   * `focused || scrollTopLifted` so both live-preview chrome and plain
   * editor routes lift the FAB; without the lift the two buttons land
   * in the same corner and the upper one swallows clicks meant for
   * the lower one.
   */
  lifted?: boolean
}
...
export function AdminScrollTopButton({ scrollRootRef, lifted = false }: AdminScrollTopButtonProps) {
  ...
  return (
    <Button
      ...
      className={cn(
        'fixed z-40',
        // When a route signals it owns a bottom-right FAB (today: the
        // editor's `FloatingPublishButton` at `right-4 bottom-6 sm:
        // bottom-8 lg:right-6`), we lift ScrollTop ~56 px higher so
        // the two stack vertically — ScrollTop on top, publish below.
        // Without the lift the two FABs land at the same anchor and
        // the upper one (here ScrollTop) swallows pointer events that
        // should reach publish. Outside editor routes no publish FAB
        // exists, so ScrollTop falls back to its historical low slot.
