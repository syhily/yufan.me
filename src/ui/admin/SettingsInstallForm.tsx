...
function pickInitialZone(timeZones: readonly string[]): string {
  ...
  if (timeZones.includes(FALLBACK_TIME_ZONE)) return FALLBACK_TIME_ZONE
  return timeZones[0] ?? FALLBACK_TIME_ZONE
}

// Stage-2 install form (site identity + asset CDN + localization).
...
export function SettingsInstallForm({ token, timeZones }: SettingsInstallFormProps) {
  ...
  return (
    <Form method="post" id="settingsInstallForm" className="tw:flex tw:flex-col tw:gap-6">
      ...
      <fieldset className="tw:flex tw:flex-col tw:gap-4 tw:border-0 tw:p-0">
        <legend className="tw:text-sm tw:font-semibold">站点信息</legend>
        ...
        <div className="tw:flex tw:flex-col tw:gap-2">
          ...
          />
          <p className="tw:text-muted-foreground tw:text-xs">完整的 https URL，结尾不带斜杠。</p>
        </div>
        <div className="tw:flex tw:flex-col tw:gap-2">
          ...
          />
          <p className="tw:text-muted-foreground tw:text-xs">评论通知 / 待审核提醒发件人，可与管理员邮箱不同。</p>
        </div>
      </fieldset>

      <fieldset className="tw:flex tw:flex-col tw:gap-4 tw:border-0 tw:p-0">
        <legend className="tw:text-sm tw:font-semibold">静态资源域名</legend>
        <div className="tw:flex tw:flex-col tw:gap-2">
          ...
          <Select
            ...
            disabled={isSubmitting}
          >
            <SelectTrigger id="install-asset-scheme" className="tw:w-full">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
      ...
      <fieldset className="tw:flex tw:flex-col tw:gap-4 tw:border-0 tw:p-0">
        <legend className="tw:text-sm tw:font-semibold">时间与本地化</legend>
        <div className="tw:flex tw:flex-col tw:gap-2">
          ...
          <Input
            ...
            placeholder="zh-CN"
          />
          <p className="tw:text-muted-foreground tw:text-xs">BCP 47 语言标签，例如 zh-CN、en-US。</p>
        </div>
        <div className="tw:flex tw:flex-col tw:gap-2">
          ...
          <Combobox<TimeZoneItem>
            ...
            onValueChange={(item) => {
              if (item) setTimeZone(item.value)
            }}
            disabled={isSubmitting}
          >
            <ComboboxTrigger id="install-timezone" className="tw:w-full">
