...
import type { AdminPostDetailDto, AdminPostDto, AdminRevisionDto, SavePostBodyOutput } from '@/shared/types/posts'
...
export function PostEditorShell({ mode, detail, navigate }: PostEditorShellProps) {
  ...
  const upsertMetaMutation = useMutation({
    ...orpcQuery.admin.posts.upsertMeta.mutationOptions(),
  ...
  const saveDraftMutation = useMutation({
    ...orpcQuery.admin.posts.saveDraft.mutationOptions(),
  ...
  const publishMutation = useMutation({
    ...orpcQuery.admin.posts.publishLatest.mutationOptions(),
    onSuccess: (payload) => {
      onBodySaved(payload)
  ...
  const unpublishMutation = useMutation({
    ...orpcQuery.admin.posts.unpublish.mutationOptions(),
    onSuccess: (payload) => {
  ...
  const flushAutosave = useCallback(
    async (snapshot: PortableTextBody) => {
      ...
      try {
        const result = await orpc.admin.posts.saveDraft({
          id: detail.post.id,
          body: snapshot,
  ...
  const persistCreate = useCallback(async () => {
    ...
    try {
      const metaResult = await orpc.admin.posts.upsertMeta({
    ...
    try {
      draftResult = await orpc.admin.posts.saveDraft({
        id: savedPost.id,
        body,
