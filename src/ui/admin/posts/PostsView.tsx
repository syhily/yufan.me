...
import type { ListCategoriesOutput } from '@/shared/categories'
import type { AdminPostDto, ListPostsOutput } from '@/shared/cms-posts'
import type { ListTagsOutput } from '@/shared/tags'
import type { AdminUserDto, ListUsersOutput } from '@/shared/users'
...
export function PostsView() {
  ...
  const deleteApi = useApiMutation((id: string) => unwrap(api.admin.posts.delete({ id })), {
    onSuccess: () => reload(),
    onError: (error) =>
      setConfirm({
        title: '删除失败',
  ...
  const restoreApi = useApiMutation((id: string) => unwrap(api.admin.posts.restore({ id })), {
    onSuccess: () => reload(),
    onError: (error) =>
      setConfirm({
        title: '恢复失败',
  ...
  const { data: usersData } = useApiQuery<ListUsersOutput>(['admin', 'users', 'options'], () =>
    unwrap(api.admin.users.list({ limit: 100, hasPosts: true })),
  )
