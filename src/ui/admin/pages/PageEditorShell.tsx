...
import type { AdminPageDetailDto, AdminPageDto, AdminRevisionDto, SavePageBodyOutput } from '@/shared/types/pages'
...
export function PageEditorShell({ mode, detail, navigate }: PageEditorShellProps) {
  ...
  // can't piggy-back on `useMutation` (which exposes a fire-and-
  // forget `mutate()` and an `onSuccess` callback). The chain awaits
  // the oRPC client directly and tracks its own pending flag.
  const [isCreatingPage, setIsCreatingPage] = useState(false)
  const upsertMetaMutation = useMutation({
    ...orpcQuery.admin.pages.upsertMeta.mutationOptions(),
  ...
  const saveDraftMutation = useMutation({
    ...orpcQuery.admin.pages.saveDraft.mutationOptions(),
  ...
  const publishMutation = useMutation({
    ...orpcQuery.admin.pages.publishLatest.mutationOptions(),
    onSuccess: (payload) => {
      onBodySaved(payload)
  ...
  const unpublishMutation = useMutation({
    ...orpcQuery.admin.pages.unpublish.mutationOptions(),
    onSuccess: (payload) => {
  ...
  // yet). The flush hits `saveDraft` directly via the oRPC client
  // so the autosave can `await` each round-trip — `useMutation`'s
  ...
  const flushAutosave = useCallback(
    async (snapshot: PortableTextBody) => {
      ...
      try {
        const result = await orpc.admin.pages.saveDraft({
          id: detail.page.id,
          body: snapshot,
  ...
  function onMetaSaved(saved: AdminPageDto) {
    // `useMutation.onSuccess` is the edit-mode path. Create mode
    // routes through `persistCreate()` instead — it awaits the oRPC
    // client directly and bypasses this callback.
    setStatus({ kind: 'saved', at: new Date() })
  ...
  const persistCreate = useCallback(async () => {
    ...
    try {
      const metaResult = await orpc.admin.pages.upsertMeta({
    ...
    try {
      draftResult = await orpc.admin.pages.saveDraft({
        id: savedPage.id,
        body,
        expectedClientRevisionToken: null,
      })
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : '保存正文失败'
      setStatus({ kind: 'error', message: errorMessage })
      setIsCreatingPage(false)
  ...
  // and are tracked independently by their `useMutation` slots.
