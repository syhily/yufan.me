...
const updateOwn = authedProc
  .route({ method: 'POST', path: '/comment-self/update-own' })
...
const requestDeleteOwn = authedProc
  .route({ method: 'POST', path: '/comment-self/request-delete-own' })
...
const cancelDeleteOwn = authedProc
  .route({ method: 'POST', path: '/comment-self/cancel-delete-own' })
  .input(z.object({ commentId: z.string() }))
  .output(successOutput)
  .handler(async ({ input, context }) => {
    const commentId = BigInt(input.commentId)
...
const listMine = authedProc
  .route({ method: 'GET', path: '/comment-self/list-mine' })
  .input(
