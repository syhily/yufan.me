...
async function buildServer(): Promise<SearchServer | null> {
  ...
  // settings snapshot. Wait for hydration before building so a
  // pre-install warmup doesn't trip `requireBlogSettingsSection()`.
  // If the deployment is uninstalled (`null`), resolve to `null` so
  // the warmup stays silent — `getServer()` will rebuild on the next
