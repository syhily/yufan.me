...
//
// Each icon under `src/assets/icons/*.svg` is eagerly imported as a raw string
...
const rawModules = import.meta.glob<string>('@/assets/icons/*.svg', {
  eager: true,
  query: '?raw',
  import: 'default',
})
...
export function Icon({ name, size, title, className }: IconProps) {
  ...
  const dim = size ?? '1em'
  const classList = cx('icon', `icon-${name}`, className)
