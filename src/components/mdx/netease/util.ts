...
const chooseUserAgent = (ua: boolean | 'mobile' | 'pc' = false) => {
  const userAgentList = {
    mobile: [
...
const cnIP = () => {
  ...
  const rnd = parseInt(String(Math.random() * (ips.length - 1 + 1)), 10);
  return ips[rnd];
};

export const request = async (method: string, url: string, data: any = {}, options: any) => {
  let headers: any = { 'User-Agent': chooseUserAgent(options.ua) };
