...
const cnip = () => {
  ...
  const arr = ips.split(',');
  const rnd = Number.parseInt(Math.random() * (arr.length - 1 - 0 + 1) + 0, 10);
  const ip = arr[rnd];
  return ip;
};
...
export const request = async (method, url, data, options) => {
  ...
  if (options.crypto === 'weapi') {
    ...
    decodedData = encrypt.weapi(data);
    decodedURL = url.replace(/\w*api/, 'weapi');
  } else if (options.crypto === 'linuxapi') {
    decodedData = encrypt.linuxapi({
      method: method,
    ...
    decodedURL = 'https://music.163.com/api/linux/forward';
    ...
    headers.Cookie = Object.keys(header)
      ...
      .join('; ');
    data.header = header;
    decodedData = encrypt.eapi(options.url, data);
    decodedURL = url.replace(/\w*api/, 'eapi');
  }

  let settings = {
    method,
    headers,
    body: new URLSearchParams(decodedData).toString(),
  };
  ...
  let count = 0;
  do {
    res = await fetch(decodedURL, settings);
    if (options.crypto === 'eapi') {
